...
let highScore=localStorage.getItem('4444-highScore');
    const high=document.getElementById("high-score")
    high.textContent=highScore;

let highScorer=localStorage.getItem('4444-highScorer');
...
document.addEventListener("keydown",(e)=>{
    if(e.code=="ArrowLeft"){
        ...
        if(setTwo()==0){
            if(canStop()){
                ...
                if(score>highScore){
                    highScore=score;
                    high.textContent=highScore;
                    localStorage.setItem('4444-highScore',highScore);
                    highScorer=player;
                    topper.innerHTML=highScorer;
                    localStorage.setItem('4444-highScorer',highScorer);
                }
            }
        ...
        if(setTwo()==0){
            if(canStop()){
                player=prompt(" GAME OVER !!, Enter Player Name: ","");
                if(score>highScore){
                    highScore=score;
                    high.textContent=highScore;
                    localStorage.setItem('4444-highScore',highScore);
